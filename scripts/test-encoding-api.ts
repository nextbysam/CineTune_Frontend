...
async function testEncodingAPI() {
	try {
		...
		try {
			...
			console.log(
				`📊 Original size: ${(stats.size / 1024 / 1024).toFixed(2)} MB\n`,
			);
		} catch {
			console.error("❌ Test video not found. Run this first:");
			console.error(
				"ffmpeg -f lavfi -i testsrc=duration=5:size=1920x1080:rate=30 -c:v libx264 -t 5 -pix_fmt yuv420p -b:v 5M -y ./public/uploads/large-test-video.mp4",
			);
			process.exit(1);
		}

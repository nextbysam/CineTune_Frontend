...
async function testEncodingAPI(videoPath: string) {
	try {
		...
		const response = await fetch(
			"http://localhost:3000/api/uploads/encode-video",
			{
				method: "POST",
				headers: {
		...
		while (!completed && attempts < maxAttempts) {
			...
			const statusResponse = await fetch(
				`http://localhost:3000/api/uploads/encode-video?jobId=${jobId}`,
			);
			const statusResult = await statusResponse.json();

			console.log(
				`Progress: ${statusResult.progress || 0}% - Status: ${statusResult.status}`,
			);
			...
			if (statusResult.status === "completed") {
				...
				if (statusResult.originalSize && statusResult.compressedSize) {
					const reduction = (
						((statusResult.originalSize - statusResult.compressedSize) /
							statusResult.originalSize) *
						100
					).toFixed(1);
				...
				throw new Error(
					`Encoding failed: ${statusResult.error || "Unknown error"}`,
				);
			}
		}
...
async function main() {
	...
	try {
		...
		console.log(
			`📊 Test video size: ${(stats.size / 1024 / 1024).toFixed(2)} MB`,
		);

		// Test the encoding API (requires the Next.js server to be running)
		console.log("\n3. Testing encoding API...");
		console.log(
			"⚠️  Make sure your Next.js development server is running (npm run dev)",
		);

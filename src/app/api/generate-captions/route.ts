  ...
  try {
    ...
    if (!contentType.includes('multipart/form-data')) {
      return NextResponse.json(
        {
          success: false,
          error: 'Invalid content type. Expected multipart/form-data with video file.',
        },
        { 
          status: 400,
          headers: {
...
async function processVideoAsync(jobId: string, videoPath: string, orientation: string, requestId: string) {
  ...
  try {
    ...
    const processingTime = Math.min(2000 + (stats.size / 1024 / 1024) * 500, 8000);
    ...
    const totalCaptions = Math.floor(estimatedDurationSeconds * captionsPerSecond);
    
    const sampleWords = [
      'Welcome', 'to', 'our', 'amazing', 'video', 'content', 'creation', 'platform',
      'Here', 'you', 'can', 'create', 'stunning', 'videos', 'with', 'professional',
      'captions', 'and', 'effects', 'Let', 'your', 'creativity', 'shine', 'through',
      'every', 'frame', 'and', 'make', 'your', 'content', 'stand', 'out',
      'from', 'the', 'crowd', 'with', 'engaging', 'storytelling', 'techniques',
      'Transform', 'your', 'ideas', 'into', 'viral', 'content', 'today'
    ];

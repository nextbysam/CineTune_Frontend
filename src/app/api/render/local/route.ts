...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log("[local-render] incoming design summary:", {
      ...
      trackItemsCount: Array.isArray(design?.trackItems) ? design.trackItems.length : 0,
    ...
    if (!design?.size || !design?.trackItems) {
      ...
      return NextResponse.json(
        { message: "Invalid design: missing size or trackItems", design: design },
        { status: 400 }
      );
    }
    ...
    try {
      ...
      console.log("[local-render] Script found, size:", scriptStats.size, "bytes");
    } catch (scriptError) {
      console.error("[local-render] Script not found:", scriptError);
      return NextResponse.json(
        { message: "Renderer script not found", scriptPath, cwd: process.cwd(), error: String(scriptError) },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      console.log("[local-render] Design file verified, size:", designStats.size, "bytes");
    } catch (designError) {
      console.error("[local-render] Design file missing:", designError);
      return NextResponse.json(
        { message: "Temp design file missing", designPath, error: String(designError) },
        { status: 500 }
      );
    }
    ...
    if (exitCode !== 0) {
      ...
      return NextResponse.json(
        { message: "Renderer failed", exitCode, stderr: stderr.slice(0, 8000), scriptPath, designPath },
        { status: 500 }
      );
    }
    ...
    try {
      ...
      return NextResponse.json(
        { message: "Invalid renderer output", stdout: stdout.slice(0, 8000), parseError: String(parseError) },
        { status: 500 }
      );
    }
    ...
    if (!filePath) {
      return NextResponse.json({ message: "No output file returned", parsed }, { status: 500 });
    }

    // Verify the output file exists
    try {
      const outputStats = await fs.stat(filePath);
      console.log("[local-render] Output file verified, size:", outputStats.size, "bytes");
    } catch (fileError) {
      console.error("[local-render] Output file missing:", fileError);
      return NextResponse.json(
        { message: "Output file not found", filePath, error: String(fileError) },
        { status: 500 }
      );
    }

...
export async function POST(request: Request) {
  ...
  try {
    ...
    console.log(`[render-start] Processing render for session: ${sanitizedSessionId}`);
    ...
    try {
      ...
      console.log("[render-start] Script found, size:", scriptStats.size, "bytes");
    } catch (scriptError) {
      console.error("[render-start] Script not found:", scriptError);
      return NextResponse.json(
        { message: "Renderer script not found", scriptPath, error: String(scriptError) },
        { status: 500 }
      );
    }
    ...
    child.on("close", async (exitCode) => {
      ...
      if (exitCode === 0) {
        try {
          ...
          console.log("[render-start] Render completed successfully:", filePath);
        } catch (parseError) {
          console.error("[render-start] Failed to parse completion output:", parseError);
          activeRenders.set(renderId, {
            ...renderData,
            status: 'error',
...
export async function GET(request: Request) {
  ...
  try {
    if (fsSync.existsSync(renderData.progressFilePath)) {
      const progressData = JSON.parse(await fs.readFile(renderData.progressFilePath, 'utf-8'));

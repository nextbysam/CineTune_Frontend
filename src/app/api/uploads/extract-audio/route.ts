...
async function getVideoOrientation(videoUrl: string): Promise<{
	...
	try {
		...
		if (!videoStream) {
			console.warn(
				`⚠️ [SERVER-AUDIO] No video stream found, defaulting to vertical`,
			);
		...
		console.log(
			`📐 [SERVER-AUDIO] Video dimensions: ${width}x${height} (${orientation})`,
		);

		return { orientation, width, height };
	} catch (error) {
		console.warn(
			`⚠️ [SERVER-AUDIO] Failed to detect orientation, defaulting to vertical:`,
			error,
		);
		return { orientation: "vertical", width: 0, height: 0 };
	}
...
async function extractAudioWithFFmpeg(
	...
	try {
		console.log(
			`🎵 [SERVER-AUDIO] Extracting audio directly from URL (no download): ${videoUrl}`,
		);
		...
		console.log(
			`⚡ [SERVER-AUDIO] Running optimized FFmpeg: ${ffmpegCmd.substring(0, 120)}...`,
		);
		...
		try {
			...
			if (stderr && stderr.length > 0) {
				...
				const progressLines = stderrLines.filter(
					(line) => line.includes("time=") && line.includes("speed="),
				);
				...
				if (errorLines.length > 0) {
					console.log(
						`⚠️ [SERVER-AUDIO] FFmpeg issues: ${errorLines.slice(0, 2).join(", ")}`,
					);
				}
			}
		} catch (ffmpegError) {
			...
			throw new Error(
				`FFmpeg execution failed: ${ffmpegError instanceof Error ? ffmpegError.message : String(ffmpegError)}`,
			);
		}
		...
		try {
			...
			console.log(
				`📊 [SERVER-AUDIO] Audio file created: ${(audioStats.size / 1024 / 1024).toFixed(2)}MB`,
			);
			...
			throw new Error(
				`Audio file was not created or is inaccessible: ${statsError instanceof Error ? statsError.message : String(statsError)}`,
			);
		}
	} catch (error) {
...
export async function POST(request: NextRequest) {
	try {
		...
		console.log(
			`🎵 [SERVER-AUDIO] Starting server-side audio extraction for video: ${videoId}`,
		);
		...
		try {
			...
			console.error(
				`❌ [SERVER-AUDIO] FFmpeg not available:`,
				ffmpegCheckError,
			);
			return NextResponse.json(
				{
					success: false,
					error:
						"Server-side audio extraction not available - FFmpeg not installed",
		...
		try {
			...
			console.log(
				`✅ [SERVER-AUDIO] FFprobe is available for orientation detection`,
			);
		} catch (ffprobeCheckError) {
			console.warn(
				`⚠️ [SERVER-AUDIO] FFprobe not available, orientation detection will be skipped:`,
				ffprobeCheckError,
			);
		...
		try {
			...
			if (ffprobeAvailable) {
				console.log(
					`📐 [SERVER-AUDIO] Getting video orientation and dimensions`,
				);
				orientationPromise = getVideoOrientation(videoUrl);
			} else {
				console.log(
					`📐 [SERVER-AUDIO] Skipping orientation detection (ffprobe not available)`,
				);
				orientationPromise = Promise.resolve({
			...
			const extractionResult = await extractAudioWithFFmpeg(
				videoUrl,
				videoId,
				options,
			);
			...
			console.log(
				`✅ [SERVER-AUDIO] Audio extraction completed in ${processingTime}ms`,
			);
			console.log(
				`📊 [SERVER-AUDIO] Audio file size: ${(extractionResult.audioSize / 1024 / 1024).toFixed(2)}MB`,
			);
			...
			if (baseUrl.includes("0.0.0.0") || baseUrl.includes("localhost")) {
				...
				if (host) {
					...
					console.log(
						`🔧 [SERVER-AUDIO] Fixed URL from ${request.nextUrl.origin} to ${baseUrl}`,
					);
				}
			}
			...
			setTimeout(
				async () => {
					try {
						...
						console.log(
							`🧹 [SERVER-AUDIO] Cleaned up temporary audio file: ${serveAudioPath}`,
						);
					} catch (cleanupError) {
						console.warn(
							`⚠️ [SERVER-AUDIO] Failed to cleanup audio file: ${cleanupError}`,
						);
					}
				},
				60 * 60 * 1000,
			...
			console.error(
				`❌ [SERVER-AUDIO] Local FFmpeg extraction failed:`,
				ffmpegError,
			);
			...
			if (ffmpegError instanceof Error) {
				...
				if (errorDetails.includes("timeout")) {
					specificError =
						"FFmpeg processing timed out - video may be too large or server overloaded";
				} else if (
					...
					specificError =
						"FFmpeg permission denied - server configuration issue";
				} else if (
					errorDetails.includes("Invalid data") ||
					errorDetails.includes("No such file")
				) {
					specificError =
						"Unable to access video URL - network or permissions issue";
				} else if (errorDetails.includes("Protocol not found")) {
					specificError =
						"FFmpeg cannot access HTTPS URLs - missing protocol support";
				}
			}
...
export async function GET() {
	try {
		...
		return NextResponse.json({
			...
			processingMethods: [
				ffmpegAvailable
					? "Local FFmpeg (primary)"
					: "Local FFmpeg (unavailable)",
			...
			capabilities: {
				formats: ["mp3", "wav", "aac"],
				maxFileSize: "500MB",
				processing: ffmpegAvailable
					? "Local FFmpeg + External fallback"

...
export async function GET(
	...
	console.log(`🟡 ==========================================`);
	console.log(
		`🟡 [${requestId}] GET /api/broll-timing-result/${brollJobId} - REQUEST STARTED`,
	);
	console.log(`🟡 ==========================================`);
	...
	try {
		...
		console.log(
			`🟡 [${requestId}] Looking up B-roll timing job: ${brollJobId}`,
		);
		const job = jobManager.getJob(brollJobId);

		if (!job) {
			console.error(
				`❌ [${requestId}] B-roll timing job not found: ${brollJobId}`,
			);
			return NextResponse.json(
				{
		...
		if (job.status === "completed" && job.brollTimings) {
			console.log(
				`✅ [${requestId}] B-roll timing job completed, timings available:`,
				job.brollTimings.length,
				"timings",
			);
		...
		console.log(`✅ [${requestId}] Returning response:`, {
			...response,
			brollTimings: response.brollTimings
				? `${response.brollTimings.length} timings`
				: "none",
		});

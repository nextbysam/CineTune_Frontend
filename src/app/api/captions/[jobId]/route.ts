...
export async function GET(
  ...
  console.log(`🔵 [${requestId}] GET /api/captions/${jobId} - Request started`);
  
  try {
    console.log(`🔵 [${requestId}] Retrieving captions for job: ${jobId}`);

    // Check if job exists
    const job = jobManager.getJob(jobId);
    console.log(`🔵 [${requestId}] Job lookup result:`, job ? 'Found' : 'Not found');
    
    if (!job) {
      console.error(`❌ [${requestId}] Job not found: ${jobId}`);
    ...
    if (job.status === 'completed' && job.captions) {
      console.log(`✅ [${requestId}] Job completed, captions available:`, job.captions.length, 'captions');
      ...
      try {
        console.log(`🔵 [${requestId}] Attempting to read captions from file...`);
        const captionsFilePath = path.join(process.cwd(), 'captions_for_remotion.json');
        console.log(`🔵 [${requestId}] File path: ${captionsFilePath}`);
        
        const captionsFileContent = await fs.readFile(captionsFilePath, 'utf-8');
        console.log(`✅ [${requestId}] File read successfully, size: ${captionsFileContent.length} bytes`);
        
        const fileCaptions = JSON.parse(captionsFileContent);
        console.log(`🔵 [${requestId}] Parsed file captions:`, fileCaptions);
        ...
        console.log(`✅ [${requestId}] Using captions from file, count: ${response.captions.length}`);
        
      } catch (fileError) {
        console.log(`⚠️ [${requestId}] No captions file found, using memory captions:`, fileError);
        response.source = 'memory';
        console.log(`🔵 [${requestId}] Using captions from memory, count: ${response.captions.length}`);
      }
      
    } else if (job.status === 'failed') {
      console.error(`❌ [${requestId}] Job failed:`, job.error);
      response.error = job.error;
      response.message = 'Caption generation failed';
    } else if (job.status === 'processing') {
      console.log(`⏳ [${requestId}] Job still processing...`);
    ...
    console.error(`❌ [${requestId}] Error retrieving captions:`, error);

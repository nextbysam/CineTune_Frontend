...
export function shouldOptimizeVideo(
	...
	console.log(
		`🔍 [VIDEO-OPT] Size check: ${videoFile.name} = ${sizeInMB.toFixed(2)}MB`,
	);
	console.log(
		`🔍 [VIDEO-OPT] Threshold: ${maxSizeMB}MB, Needs optimization: ${needsOptimization}`,
	);
...
export async function optimizeVideoForCaptions(
	...
	if (!shouldOptimizeVideo(videoFile, maxSizeMB)) {
		console.log(
			`✅ [VIDEO-OPT] No optimization needed - file under ${maxSizeMB}MB threshold`,
		);
		return {
	...
	if (
		preferAudioExtraction &&
		shouldExtractAudio(videoFile, audioExtractionThreshold)
	) {
		console.log(
			`🎵 [VIDEO-OPT] Attempting audio extraction (primary strategy)`,
		);
		try {
			...
			console.warn(
				`⚠️ [VIDEO-OPT] Audio extraction failed, trying video compression:`,
				audioError,
			);
		}
	}
	...
	try {
		...
		console.log(
			`🎬 [VIDEO-OPT] Starting video compression optimization for ${videoFile.name} (${originalSizeInMB.toFixed(2)}MB)`,
		);
		...
		console.log(
			`📐 [VIDEO-OPT] Video dimensions: ${originalWidth}x${originalHeight}, duration: ${duration.toFixed(2)}s`,
		);
		...
		console.log(
			`🔄 [VIDEO-OPT] Initial target dimensions: ${targetWidth}x${targetHeight}, aspect ratio: ${aspectRatio.toFixed(3)}`,
		);

		if (targetWidth / targetHeight > aspectRatio) {
			targetWidth = targetHeight * aspectRatio;
			console.log(
				`📏 [VIDEO-OPT] Adjusted width to maintain aspect ratio: ${targetWidth}`,
			);
		} else {
			targetHeight = targetWidth / aspectRatio;
			console.log(
				`📏 [VIDEO-OPT] Adjusted height to maintain aspect ratio: ${targetHeight}`,
			);
		}
		...
		console.log(
			`📏 [VIDEO-OPT] Final target dimensions (rounded): ${originalWidth}x${originalHeight} → ${targetWidth}x${targetHeight}`,
		);
		...
		try {
			...
			console.log(
				`🎛️ [VIDEO-OPT] Settings: ${videoBitrate}kbps video, ${audioBitrate}kbps audio, quality: ${quality}`,
			);
			...
			console.warn(
				`⚠️ [VIDEO-OPT] MediaRecorder compression failed, using fallback strategy:`,
				mediaRecorderError,
			);
			...
			optimizedFile = await createSimplifiedVideo(
				videoFile,
				targetWidth,
				targetHeight,
				quality,
		...
		const compressionRatio = Math.round(
			((videoFile.size - optimizedSize) / videoFile.size) * 100,
		);
		const processingTime = Date.now() - startTime;
		...
		try {
			const chunkResult = await createSimplifiedVideo(
				videoFile,
				maxWidth,
				maxHeight,
				quality,
			...
			if (chunkResult.size < videoFile.size) {
				const compressionRatio = Math.round(
					((videoFile.size - chunkResult.size) / videoFile.size) * 100,
				);
				console.log(
					`✅ [VIDEO-OPT] File chunking successful: ${compressionRatio}% reduction`,
				);
		...
		const processingTime = Date.now() - startTime;
		console.log(
			`⚠️ [VIDEO-OPT] All optimization strategies failed, returning original file after ${(processingTime / 1000).toFixed(1)}s`,
		);
...
async function compressVideoWithMediaRecorder(
	...
	console.log(
		`🎬 [MEDIA-REC] Target dimensions: ${targetWidth}x${targetHeight}`,
	);
	console.log(
		`🎬 [MEDIA-REC] Bitrates: video ${videoBitrate}kbps, audio ${audioBitrate}kbps`,
	);
	...
	return new Promise((resolve, reject) => {
		try {
			...
			console.log(
				`⚙️ [MEDIA-REC] MediaRecorder settings: video ${videoBitrate * 1000}bps, audio ${audioBitrate * 1000}bps`,
			);
			...
			console.log(
				`📝 [MEDIA-REC] MediaRecorder configured, setting up event handlers`,
			);

			mediaRecorder.ondataavailable = (event) => {
				if (event.data.size > 0) {
					console.log(
						`📦 [MEDIA-REC] Received chunk: ${(event.data.size / 1024).toFixed(1)}KB`,
					);
			...
			mediaRecorder.onstop = () => {
				console.log(
					`🛑 [MEDIA-REC] MediaRecorder stopped, processing ${chunks.length} chunks`,
				);
				try {
					// Clean up
					...
					console.log(
						`✅ [MEDIA-REC] Created optimized file: ${fileName} (${(blob.size / 1024 / 1024).toFixed(2)}MB)`,
					);
			...
			const drawFrame = () => {
				if (video.ended || video.paused) {
					console.log(
						`🎬 [MEDIA-REC] Video playback ended, stopping MediaRecorder (${frameCount} frames processed)`,
					);
					mediaRecorder.stop();
					return;
				}
				...
				if (frameCount % 30 === 0) {
					...
					console.log(
						`🎬 [MEDIA-REC] Processed ${frameCount} frames (${(video.currentTime).toFixed(1)}s/${video.duration.toFixed(1)}s)`,
					);
				}
...
async function createSimplifiedVideo(
	...
	console.log(
		`📁 [FALLBACK] Original file: ${originalFile.name} (${(originalFile.size / 1024 / 1024).toFixed(2)}MB)`,
	);
	...
	try {
		...
		console.log(
			`🎯 [FALLBACK] Target chunk size: ${maxChunkSizeMB}MB (${maxChunkSizeBytes} bytes)`,
		);

		if (originalFile.size <= maxChunkSizeBytes) {
			// File is already small enough
			console.log(
				`✅ [FALLBACK] File already under ${maxChunkSizeMB}MB, no chunking needed`,
			);
			return originalFile;
		}
		...
		console.log(
			`📦 [FALLBACK] ArrayBuffer created: ${(arrayBuffer.byteLength / 1024 / 1024).toFixed(2)}MB`,
		);

		const chunkSize = Math.min(arrayBuffer.byteLength, maxChunkSizeBytes);
		console.log(
			`✂️ [FALLBACK] Creating chunk of ${(chunkSize / 1024 / 1024).toFixed(2)}MB from beginning`,
		);
		const chunk = arrayBuffer.slice(0, chunkSize);

		const fileName = originalFile.name.replace(
			/\.[^/.]+$/,
			"_optimized_chunk.mp4",
		);
		const optimizedFile = new File([chunk], fileName, {
			type: originalFile.type,
		});

		console.log(`✅ [FALLBACK] Created chunk file: ${fileName}`);
		console.log(
			`📊 [FALLBACK] Size reduction: ${(originalFile.size / 1024 / 1024).toFixed(2)}MB → ${(optimizedFile.size / 1024 / 1024).toFixed(2)}MB`,
		);

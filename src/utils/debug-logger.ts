...
class DebugLogger {
	...
	public logRenderStart(payload: any): void {
		...
		console.log(
			...
			systemInfo.browser.hardwareConcurrency,
			"cores",
		);
		...
		console.log(
			"Viewport:",
			`${systemInfo.viewport.width}x${systemInfo.viewport.height}`,
		);
		console.log(
			"Screen:",
			`${systemInfo.viewport.screenWidth}x${systemInfo.viewport.screenHeight}`,
		);
		...
		if (payload) {
			...
			console.log(
				"Video Dimensions:",
				payload.size
					? `${payload.size.width}x${payload.size.height}`
					: "Not specified",
			);
			...
			console.log(
				"Duration:",
				payload.duration
					? `${payload.duration}ms (${(payload.duration / 1000).toFixed(2)}s)`
					: "Not specified",
			);

			const trackItems = Array.isArray(payload.trackItems)
				? payload.trackItems
				: [];
			...
			if (trackItems.length > 0) {
				...
				const videoItems = trackItems.filter(
					(item: any) => item.type === "video",
				);
				const audioItems = trackItems.filter(
					...
					videoItems.forEach((video: any, index: number) => {
						...
						console.log(`Video ${index + 1}:`, {
							...
							duration: video.display
								? `${video.display.to - video.display.from}ms`
								: "Unknown",
					...
					audioItems.forEach((audio: any, index: number) => {
						console.log(`Audio ${index + 1}:`, {
							...
							duration: audio.display
								? `${audio.display.to - audio.display.from}ms`
								: "Unknown",
						});
					});
					console.groupEnd();
	...
	public logError(error: any, context: string = "Unknown"): void {
		...
		if (
			...
			console.error(
				"🏷️ TYPE ERROR - Incorrect data type or null/undefined access",
			);
		} else if (

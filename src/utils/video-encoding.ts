...
export interface EncodingCallbacks {
	...
	onComplete?: (
		jobId: string,
		result: { outputUrl: string; originalSize: number; compressedSize: number },
	) => void;
}
...
export async function getEncodingStatus(jobId: string): Promise<EncodingJob> {
	try {
		const response = await axios.get(
			`/api/uploads/encode-video?jobId=${jobId}`,
		);
...
export async function processVideoFile(
	...
	return new Promise(async (resolve, reject) => {
		try {
			console.log(
				`📁 Processing video file: ${file.name} (${(file.size / 1024 / 1024).toFixed(2)} MB)`,
			);
 ...
 */
export function shouldEncodeVideo(
	file: File,
	maxSizeForEncoding: number = 25 * 1024 * 1024,
): boolean {
	...
	const highBitrateFormats = [
		"video/mov",
		"video/quicktime", // iPhone/Mac recordings
		"video/avi", // Uncompressed AVI
		"video/x-msvideo", // AVI variant
		"video/mts",
		"video/m2ts", // AVCHD formats
		"video/x-ms-wmv", // Windows Media Video
		"video/3gpp",
		"video/3gp2", // Mobile formats
	];

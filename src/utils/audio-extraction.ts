...
export function shouldExtractAudio(
  ...
  console.log(`🎵 [AUDIO-EXTRACT] Size check: ${videoFile.name} = ${sizeInMB.toFixed(2)}MB`);
  console.log(`🎵 [AUDIO-EXTRACT] Threshold: ${minVideoSizeMB}MB, Should extract audio: ${shouldExtract}`);
...
export async function extractAudioForCaptions(
  ...
  console.log(`📁 [AUDIO-EXTRACT] Input: ${videoFile.name} (${(videoFile.size / 1024 / 1024).toFixed(2)}MB)`);
  ...
  try {
    ...
    await new Promise<void>((resolve, reject) => {
      video.onloadedmetadata = () => {
        console.log(`✅ [AUDIO-EXTRACT] Metadata loaded - Duration: ${video.duration.toFixed(2)}s`);
        resolve();
      };
      video.onerror = (error) => {
        console.error(`❌ [AUDIO-EXTRACT] Failed to load video metadata:`, error);
    ...
    if (!hasAudio) {
      console.warn(`⚠️ [AUDIO-EXTRACT] Video has no audio track, returning original file`);
    ...
    console.log(`🎙️ [AUDIO-EXTRACT] Starting audio extraction with MediaRecorder`);
    ...
    const compressionRatio = Math.round(((videoFile.size - audioFile.size) / videoFile.size) * 100);
    const processingTime = Date.now() - startTime;

    console.log(`📊 [AUDIO-EXTRACT] Extraction complete:`);
    console.log(`   📏 Original video: ${(videoFile.size / 1024 / 1024).toFixed(2)}MB`);
    console.log(`   🎵 Extracted audio: ${(audioFile.size / 1024 / 1024).toFixed(2)}MB`);
...
async function checkVideoHasAudio(video: HTMLVideoElement): Promise<boolean> {
  ...
  try {
    ...
    if (anyVideo.audioTracks && anyVideo.audioTracks.length > 0) {
      console.log(`✅ [AUDIO-EXTRACT] Audio detected via audioTracks API (${anyVideo.audioTracks.length} tracks)`);
      return true;
    }
    ...
    if (typeof anyVideo.mozHasAudio !== 'undefined') {
      ...
      console.log(`✅ [AUDIO-EXTRACT] Audio detected via mozHasAudio: ${hasAudio}`);
      return hasAudio;
    }
    ...
    if (typeof anyVideo.webkitAudioDecodedByteCount !== 'undefined') {
      ...
      console.log(`✅ [AUDIO-EXTRACT] Audio detected via webkitAudioDecodedByteCount: ${hasAudio}`);
      return hasAudio;
    }
    ...
    console.log(`🔍 [AUDIO-EXTRACT] AudioContext detection result: ${hasAudioData}`);
    return hasAudioData;

  } catch (error) {
    console.warn(`⚠️ [AUDIO-EXTRACT] Audio detection failed, assuming audio exists:`, error);
...
async function extractAudioWithMediaRecorder(
  ...
  console.log(`⚙️ [MEDIA-REC-AUDIO] Settings: ${audioBitrate}kbps, format: ${audioFormat}, max: ${maxDuration}s`);
    ...
    try {
      ...
      mediaRecorder.ondataavailable = (event) => {
        if (event.data.size > 0) {
          console.log(`📦 [MEDIA-REC-AUDIO] Received audio chunk: ${(event.data.size / 1024).toFixed(1)}KB`);
          chunks.push(event.data);
        }
      };

      mediaRecorder.onstop = () => {
        console.log(`🛑 [MEDIA-REC-AUDIO] Recording stopped, processing ${chunks.length} chunks`);
        ...
        try {
          ...
          const fileName = originalFileName.replace(/\.[^/.]+$/, `_audio.${audioFormat}`);
          const audioFile = new File([blob], fileName, { type: mimeType });
          
          console.log(`✅ [MEDIA-REC-AUDIO] Audio file created: ${fileName} (${(blob.size / 1024 / 1024).toFixed(2)}MB)`);
          resolve(audioFile);
        } catch (error) {
          console.error(`❌ [MEDIA-REC-AUDIO] Error creating audio file:`, error);
          reject(error);
        }
      };
      ...
      video.currentTime = 0;
      video.play().then(() => {
        console.log(`▶️ [MEDIA-REC-AUDIO] Video playback started for audio extraction`);
      }).catch(reject);

      // Stop recording when video ends or max duration reached
      video.onended = () => {
        console.log(`🏁 [MEDIA-REC-AUDIO] Video ended, stopping audio recording`);
      ...
      setTimeout(() => {
        if (mediaRecorder.state === 'recording') {
          console.log(`⏰ [MEDIA-REC-AUDIO] Max duration (${maxDuration}s) reached, stopping recording`);
          video.pause();
          mediaRecorder.stop();
        }
      }, maxDuration * 1000);
...
function getAudioMimeType(format: string): string {
  ...
  // Fallback
  console.warn(`⚠️ [AUDIO-EXTRACT] No supported MIME type found, using fallback: audio/webm`);
  return 'audio/webm';
}
